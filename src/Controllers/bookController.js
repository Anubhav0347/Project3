...
const createBook = async function (req, res) {
    try {
        ...
        if(!isValidRequest(bookData)) return res.status(400).send({status: false, message:"body is empty"});
        const {title,excerpt,userId,category,subcategory,ISBN,releasedAt}=bookData
       //*********Title VALIDATIONS**************8 */
        if(!isValid(title)) return res.status(400).send({status: false, message:"title is mandatory and valid"});
        if(!stringRegex.test(title)) return res.status(400).send({status: false, message:"title can conatin only alphabets"})
        
        const titleData=await bookModel.findOne({title:title})
           if(titleData) return res.status(400).send({status:false,message:"title already exist "})
  
        
        //***********Excerpt VALIDATIONS**********8 */
  
        if(!isValid(excerpt)) return res.status(400).send({status: false, message:"excerpt is mandatory and valid"});
          //***********USERID VALIDATION************ */
       ...
       if(!isValid(category)) return res.status(400).send({status: false, message:"category is mandatory and valid"});
       if(!stringRegex.test(category)) return res.status(400).send({status: false, message:"category can conatin only alphabets"})
       
       if(!isValid(subcategory)) return res.status(400).send({status: false, message:"subcategory is mandatory and valid"});
       if(!stringRegex.test(subcategory)) return res.status(400).send({status: false, message:"subcategory can conatin only alphabets"})
         
       //**********Reviews Validations*************** */
        
         if(bookData.reviews){
            if(bookData.reviews!==0) return res.status(400).send({status:false,message:"reviews should not be greater than zero or less than"})
         }
         //***********ISBN Validations******************** */
        
         if(!bookData.ISBN) return res.status(400).send({status:false,message:"ISBN not present"})
  
         if(!(ISBNregex.test(ISBN)))  //(?=(?:\D*\d){10}(?:(?:\D*\d){3})?$)
            return res.status(400).send({status: false, message:" ISBN is not in proper format"});
        ...
        return res.status(201).send({ status: true, message: "book created successfully", data: saveData });
    } catch (error) {
...
const getBooks = async function (req, res) {
    try {
        if (isValidRequest(req.body))
            return res.status(400).send({ status: false, message: "filters can passes only through query params" })
        ...
        if (isValidRequest(queryParams)) {
            ...
            return res.status(200).send({ status: true, message: "filtered book list", booksCount: filetredBooks.length, bookList: filetredBooks.sort((a, b) => a.title.localeCompare(b.title)) })
                  ...
                  return  res.status(200).send({ status: true, message: "book list", booksCount: allBooks.length, booksList: allBooks.sort((a, b) => a.title.localeCompare(b.title))});
        }
...
const deleteBook=async function (req,res){
    try {
        ...
        if(isValidRequest(reqbody)) return res.status(400).send({status:false,message:"invalid request"})
        ...
        if(!bookbyBookId) return res.status(400).send({status:false,message: `no blog found by ${bookId}`})
        await bookModel.findByIdAndUpdate({ _id: bookId },{ $set: { isDeleted: true, deletedAt: Date.now() } },{ new: true })
